...
//#define NEXT_INSTRUCTION do {goto *opcode[memory[PC++]&IRQ]; } while(0)
#define NEXT_INSTRUCTION do {instcount++;goto *opcode[memory[PC++]&IRQ]; } while(0)

volatile unsigned int instcount;
...
#define ABSOLUTE_Y asm("movw _memory(%1),%0; addw %2,%0" \
  ...
  while (ncycles--)
    ...
    opcode_00: /* BRK */
      // real IRQ requires current instruction to be reloaded.
      if(reset){
        P = 0x20; /* The unused bit is always 1 */
        S = 0xff;
